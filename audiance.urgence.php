...
$data=$a->fetch();
  ...
  extract($_POST);
        ...
        $ps=$bdd->prepare("INSERT INTO audiance(motif_audiance,heur_audiance,min_audiance,jour_audiance,mois_audiance,annee_audiance,observation_audiance,id_correspondant,id_agent,id_agent_recois,traite_audiance) VALUES (?,?,?,?,?,?,?,?,?,?,?)") or die("erreur req");
        $param=array($motif_audiance,$heur_audiance,$min_audiance,$jour_audiance,$mois_audiance,$annee_audiance,$observation_audiance,$id_correspondant,$_SESSION["id_agent"],$id_agent,"0") or die("erreur param");
            $ps->execute($param);
